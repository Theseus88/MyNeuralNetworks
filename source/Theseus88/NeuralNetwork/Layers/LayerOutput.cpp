...
namespace Theseus88 {
    ...
    template <typename T> void LayerOutput<T>::loadNetworkLayer(JsonReader& reader) {
        ...
        for (std::size_t i = 0; i < LayerBase<T>::m_neuronCount; ++i) {
            ...
            std::string nTypeStr = reader.readString("Neuron Type");
            NeuronType nType = stringToNeuronType(nTypeStr);
