...
namespace Theseus88 {
    ...
    template <typename T> std::function<const T(NeuralNetworkNeuron<T>&, const T, const T)> ErrorFunctions<T>::getErrorFunction(const Method method) {
        switch (method) {
    ...
    // ADD COMMENT HERE LATER
    template <typename T> const T ErrorFunctions<T>::meanSquaredError(NeuralNetworkNeuron<T>& neuron, const T predicted, const T target) {
        return predicted - target;
    };
    template <typename T> const T ErrorFunctions<T>::meanAbsoluteError(NeuralNetworkNeuron<T>& neuron, const T predicted, const T target) {
        return (predicted > target) ? static_cast<T>(1.0) : (predicted < target) ? static_cast<T>(-1.0) : static_cast<T>(0.0);
    };
    template <typename T> const T ErrorFunctions<T>::squaredError(NeuralNetworkNeuron<T>& neuron, const T predicted, const T target) {
        return 2 * (predicted - target);
    };
    template <typename T> const T ErrorFunctions<T>::binaryCrossEntropy(NeuralNetworkNeuron<T>& neuron, const T predicted, const T target) {
        const T& epsilon = neuron.getEpsilon();
        return -(target / (predicted + epsilon)) + ((1 - target) / (1 - predicted + epsilon));
    };
    template <typename T> const T ErrorFunctions<T>::hingeLoss(NeuralNetworkNeuron<T>& neuron, const T predicted, const T target) {
        return (1 - target * predicted < 0) ? static_cast<T>(0.0) : -target;
    };
    template <typename T> const T ErrorFunctions<T>::focalLoss(NeuralNetworkNeuron<T>& neuron, const T predicted, const T target) {
        const T& epsilon = neuron.getEpsilon();
